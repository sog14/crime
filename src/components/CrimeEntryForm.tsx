...
import { getAutocompleteSuggestions, getPlaceDetails, AutocompleteSuggestion } from '../utils/rapidPlaces';
...
export function CrimeEntryForm({ onClose, onSuccess, initialData }: CrimeEntryFormProps) {
  ...
  // Debounced places autocomplete API call and instant curated spots
  useEffect(() => {
    if (searchQuery.trim().length === 0) {
      getAutocompleteSuggestions('', location).then(results => {
    ...
    const timer = setTimeout(async () => {
      try {
        const results = await getAutocompleteSuggestions(searchQuery, location);
        setSuggestions(results);
  ...
  const handleSelectSuggestion = async (suggestion: AutocompleteSuggestion) => {
    ...
    try {
      ...
      if (details && details.location) {
        ...
        // Update the form address text area
        setValue('address', details.formattedAddress || text);
      } else {
        alert('Details for selected location not found. Try manually dragging the marker.');
      }
    } catch (err) {
      console.error('Error fetching details of selection:', err);
    } finally {
      setSearching(false);
    }
  };
  ...
  const handleSearchPlace = async () => {
    // Legacy fallback, trigger first suggestion if available
    if (suggestions.length > 0) {
      handleSelectSuggestion(suggestions[0]);
    } else if (searchQuery.trim()) {
      // Fallback geocoding or alerting
      alert('Selected location not found, please select from the dropdown options as you type.');
    }
  };
              ...
              <div className="bg-zinc-50 rounded-[2rem] p-6 border border-zinc-100 space-y-6">
                ...
                <div className="space-y-2 relative">
                  ...
                  {showDropdown && suggestions.length > 0 && (
                    <div className="absolute left-0 right-0 top-full mt-2 bg-white rounded-2xl shadow-xl border border-zinc-150 overflow-hidden z-[50] max-h-60 overflow-y-auto">
                      {suggestions.map((item) => (
                        <button
                          key={item.placePrediction.placeId}
                          type="button"
                          onClick={() => handleSelectSuggestion(item)}
                          className="w-full px-4 py-3 text-left hover:bg-zinc-50 transition-colors border-b border-zinc-50 last:border-0 flex items-start gap-2"
                        >
